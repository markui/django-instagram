...
def facebook_login(request):
    # NamedTuple로 하는 이유: 맨날 response.json() 찍으면서 속성에 무엇이 있는지 찍어볼 필요가 없고
    # 명시적으로 어떠한 속성들을 꺼내 쓸 수 있는지를 확인할 수 있음
    # 개발에 편리함
    ...
    # 1. 전달받은 code값으로 AccessTokenInfo namedtuple을 반환
    access_token_info = get_access_token_info(code)
    # 2. namedtuple에서 'access_token'속성의 값을 가져옴
    access_token = access_token_info.access_token
    # 3. DebugTokenInfo 가져오기 => 보통 프론트엔드에서 access_token 까지 갖고와서
    # 요청을 백엔드로 REST로 해주면, 한 번 더 debug해주는 게 보안에 좋다
